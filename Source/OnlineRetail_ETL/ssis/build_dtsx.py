...
def main():
 ...
 r=ET.Element(q(D,'Executable'),{q(D,'refId'):'Package',q(D,'CreationName'):'Microsoft.Package',q(D,'DTSID'):uid(),q(D,'ExecutableType'):'Microsoft.Package',q(D,'LastModifiedProductVersion'):'16.0.4215.2',q(D,'LocaleID'):'1033',q(D,'ObjectName'):'00_Master_OnlineRetail_ETL',q(D,'PackageType'):'5',q(D,'VersionBuild'):'1',q(D,'VersionGUID'):uid(),q(D,'ProtectionLevel'):'0'})
 p=ET.SubElement(r,q(D,'Property'),{q(D,'Name'):'PackageFormatVersion'}); p.text='8'
 cms=ET.SubElement(r,q(D,'ConnectionManagers')); cm=ET.SubElement(cms,q(D,'ConnectionManager'),{q(D,'refId'):'Package.ConnectionManagers[RetailDW]',q(D,'CreationName'):'OLEDB',q(D,'DTSID'):cid,q(D,'ObjectName'):'RetailDW'}); od=ET.SubElement(cm,q(D,'ObjectData')); ET.SubElement(od,q(D,'ConnectionManager'),{q(D,'ConnectRetryCount'):'3',q(D,'ConnectRetryInterval'):'2',q(D,'ConnectionString'):'Provider=MSOLEDBSQL;Data Source=localhost;Initial Catalog=RetailDW;User ID=sa;Password=RUNTIME_OVERRIDE;Encrypt=Optional;TrustServerCertificate=True;'})
 ET.SubElement(r,q(D,'Variables')); xs=ET.SubElement(r,q(D,'Executables'))
 steps=[('01 - Begin ETL batch','EXEC etl.usp_BeginBatch;'),('02 - Extract CSV to staging','EXEC etl.usp_LoadStaging;'),('03 - Transform and load dimensions','EXEC etl.usp_LoadDimensions;'),('04 - Transform and load sales fact','EXEC etl.usp_LoadFact;'),('05 - Validate and close batch','EXEC etl.usp_ValidateAndCloseBatch;')]; refs=[]
